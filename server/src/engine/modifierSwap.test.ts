...
describe('обмен счётом в комнате на одного', () => {
  it('не ждёт выбора цели и не записывает открывшего своей же целью', () => {
    const state = openedSoloSwap();
    expect(state.phase).toBe('modifier');
    // targetPlayerId остаётся null — цели нет и не будет, но null не значит
    // «пока не выбрал», а значит «выбирать некого». Открывший не может стать
    // собственной целью: это значение MODIFIER_TARGET сам же запрещает («С
    // самим собой меняться нельзя»), и state.modifier не должен был бы
    // хранить то, что правила игры признают невозможным.
    expect(state.modifier?.targetPlayerId).toBeNull();
    expect(state.modifier?.targetPlayerId).not.toBe(state.modifier?.playerId);
  ...
  it('выбор себя же целью отклоняется, как и в обычном обмене', () => {
    // Телефон открывшего не покажет кнопку выбора (кандидатов нет), но
    // движок всё равно защищён: случайный/устаревший запрос с целью на
    // самого себя отвергается тем же правилом, что и в комнате с игроками.
