    ...
    <div className="grid gap-6">
      {(errors.length > 0 || warnings.length > 0) && (
        <section className="on-paper ink-border bg-card text-ink relative grid gap-2 rounded-2xl p-4">
          <h2 className="font-body text-sm font-bold">
            Проверка пака: <span className="text-no-ink tabular-nums">{errors.length}</span> ошибок,{' '}
            <span className="text-p1-ink tabular-nums">{warnings.length}</span> предупреждений
          </h2>
          <ul className="grid max-h-40 gap-1 overflow-y-auto text-sm">
            {[...errors, ...warnings].slice(0, 30).map((issue, index) => (
              <li
                key={`${issue.message}-${index}`}
                className={issue.level === 'error' ? 'text-no-ink font-bold' : 'text-soft'}
              >
                {issue.message}
              </li>
            ))}
          ...
          {errors.length > 0 && (
            <p className="text-soft text-xs">Пока есть ошибки, комнату на этом паке создать нельзя.</p>
          )}
        </section>
      )}
        ...
        <section key={round.id} className="grid gap-3">
          <header className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              aria-expanded={pasteInto === round.id}
            ...
            {pack.rounds.length > 1 && (
              <button
                ...
                className="btn btn-quiet font-body text-sm hover:border-no hover:text-no-ink"
              >
                Удалить раунд
              </button>
            )}
      ...
      <button
        type="button"
        onClick={addRound}
        className="btn btn-dashed font-body justify-self-start"
      >
        + раунд
      </button>

      <section className="grid gap-3">
        <h2 className="font-pop text-xl font-black">Финал</h2>
        ...
        {pack.final.themes.map((theme) => (
          <div
            key={theme.id}
            className="on-paper ink-border bg-card text-ink relative grid gap-2 rounded-2xl p-4"
          >
            <div className="flex items-center gap-2">
              <input
                className="field-bare min-w-0 flex-1 font-bold"
                value={theme.title}
              ...
              <button
                ...
                aria-label={`Удалить финальную тему «${theme.title}»`}
                title="Удалить финальную тему"
                className="btn text-soft shrink-0 px-2 text-sm hover:border-no hover:text-no-ink"
              >
                ✕
              </button>
            </div>
